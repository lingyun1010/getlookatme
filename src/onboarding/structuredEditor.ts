...
export function mountStructuredProfileEditor(host:HTMLElement,initial:ProfileDocumentDraft,onSave:SaveDraft){
  ...
  function render(){
    ...
    if(editing==='hero'){
      const form=node('form','structured-inline-editor');form.onsubmit=event=>event.preventDefault();heroSpecs.forEach(spec=>form.append(inputField(spec,draft.identity[spec.key as keyof typeof draft.identity]??'')))
      form.append(editorActions(()=>{editing=null;render()},()=>void (async()=>{if(!form.reportValidity())return;const next=clone(draft);next.identity={...next.identity,...readFields(form,heroSpecs)};await update(next)})()))
      hero.append(form)
    ...
    if(editing==='skills'){
      const form=node('form','structured-inline-editor skill-input-list');form.onsubmit=event=>event.preventDefault();const values=draft.skills.flatMap(group=>group.items);const rows=[...values,''];rows.forEach((value,index)=>{const field=inputField({key:`skill-${index}`,label:`Skill ${index+1}`},value);form.append(field)})
      form.append(editorActions(()=>{editing=null;render()},()=>void (async()=>{const next=clone(draft);const items=Array.from(form.querySelectorAll<HTMLInputElement>('input')).map(input=>input.value.trim()).filter(Boolean);next.skills=items.length?[{id:'profile-skills',category:'Skills',items}]:[];await update(next)})()));skills.append(form)
