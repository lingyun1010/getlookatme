...
export function createAiLifecycleDependencies():AiLifecycleDependencies{const db=client();return{state:new SupabaseAiStateRepository(db),knowledge:new SupabaseKnowledgeRepository(db),embeddings:{version:embeddingVersion(),embedTexts:inputs=>new OpenAIEmbeddingClient().embedTexts(inputs),embedText:input=>new OpenAIEmbeddingClient().embedText(input)},now:()=>new Date().toISOString()}}
