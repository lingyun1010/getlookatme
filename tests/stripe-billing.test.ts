...
test('direct stripe cancellation schedules cancel_at_period_end for the authenticated user only', async () => {
  const request = await readFile(new URL('../src/billing/request.ts', import.meta.url), 'utf8')
  const provider = await readFile(new URL('../src/billing/stripe.ts', import.meta.url), 'utf8')
  const webhook = await readFile(new URL('../src/billing/stripeWebhook.ts', import.meta.url), 'utf8')
  const migration = await readFile(new URL('../supabase/migrations/20260922100000_subscription_cancel_at_period_end.sql', import.meta.url), 'utf8')
  assert.match(request, /action !== 'cancel-subscription'/)
  assert.match(request, /scheduleCancellation\(user\.id\)/)
  assert.match(request, /resumeSubscription\(user\.id\)/)
  assert.doesNotMatch(request, /body\.subscriptionId|body\.provider_subscription_id|body\.userId/)
  assert.match(provider, /cancel_at_period_end: true/)
  assert.match(provider, /cancel_at_period_end: false/)
  assert.match(provider, /subscriptions\.update\(subscription\.provider_subscription_id/)
  assert.match(provider, /Founding access is managed manually/)
